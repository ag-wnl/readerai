document.addEventListener("DOMContentLoaded", function () {
  ...
  recents.addEventListener("click", function () {
    if (recent_list.querySelectorAll("li").length > 0) {
      return; //if recent notes already open, dont append recent notes again
    }
    ...
    recent_list.addEventListener("click", function (event) {
      if (event.target.tagName === "LI") {
        ...
        function openNotesPage() {
          ...
          window.open(
            pageURL + "?note=" + ele_id + "&exists=" + exists,
            "_blank"
          );
        }
